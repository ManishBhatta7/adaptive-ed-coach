...
export const AITutorSystem: React.FC = () => {
  ...
  const sendToTutor = async (input: string, interactionType: string = 'question') => {
    ...
    try {
      const systemContext = `
        You are an AI Tutor teaching ${currentSession.context_data?.topic || 'General Topics'}.
        Your personality is: ${currentSession.context_data?.tutor_personality || 'Encouraging'}.
        Current Difficulty Level: ${currentSession.difficulty_level}.
        User Goal: ${currentSession.context_data?.goal || 'Learn and improve'}.
        
        Task: Provide a helpful, educational response to the student's input.
        - If the input is a question, answer it clearly.
        - If it's an assessment request, provide a short quiz.
      ...
      if (aiError) {
        throw new Error(aiError.message || 'Failed to connect to AI Tutor');
      }

      const aiResponseText = aiData?.response || "I'm having trouble thinking right now. Please try again.";
      ...
      if (dbError) {
        console.error('Error saving interaction history:', dbError);
      }
      ...
      toast({
        ...
        description: error.message || "Could not reach the AI Tutor. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsTyping(false);
